<!doctype html>
<html lang="es_MX">
  <head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
    <link rel="stylesheet" type="text/css" href="estilos.css">
    <link href="https://fonts.googleapis.com/css?family=Roboto" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Roboto&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="http://maxcdn.bootstrapcdn.com/font-awesome/4.2.0/css/font-awesome.min.css">
    <link href="http://fonts.googleapis.com/css?family=Cookie" rel="stylesheet" type="text/css">

    <title>Intop Solutions</title>
  </head>
  <body>
    
    <nav style="background-image: url('fondo-arriba.png');background-repeat: no-repeat;background-size:100% 100%;" class="navbar navbar-expand-lg navbar-dark barraNav">
    <div class="container" style="margin-left: 0px;">
      <a  class="navbar-left" href="index.php">
             <img style="max-width:100%; max-height:100%;" src="Logo intop_blanco.svg" alt="">
          </a>
      <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarResponsive" aria-controls="navbarResponsive" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
          </button>
      <div class="collapse navbar-collapse" id="navbarResponsive">
        <ul class="navbar-nav mx-auto">
          <li class="nav-item">
            <a class="nav-link" style="color : white" href="index.php">INICIO</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" style="color : white;border-bottom: 2px solid #FFFFFF;" href="productos.php">
                PRODUCTOS
            <span class="sr-only">(current)</span></a>
          </li>
          <li class="nav-item">
            <a class="nav-link" style="color : white" href="servicios.php">SERVICIOS</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" style="color : white" href="nosotros.php">NOSOTROS</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" style="color : white" href="contacto.php">CONTACTO</a>
          </li>
        </ul>
      </div>
    </div>
</nav>

<br>

<div  style="text-align:center;width:100%;">
  <div class="row" style="display: flex;width:100%;">
    <div class="col-sm-9 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;padding-left:5%;">
      <div>
          <h2 style="font-family: 'Roboto', sans-serif; font-weight: bold; text-align: left;padding-top: 5%;padding-bottom: 10px;color: #002C55;">
            NUESTRA LÍNEA DE PRODUCTOS
          </h2>
            <h5 style="font-family: 'Roboto', sans-serif;
                  	    font-weight: medium;
                  	    text-align: left;
                    	padding-bottom: 10px;
                    	color: #002E54;">
                   FÁCILES DE USAR · CONFIABLES · PRÁCTICOS · INNOVADORES · ÚTILES
            </h5>
            <h5 style="font-family: 'Roboto', sans-serif;
                  	    font-weight: normal;
                  	    text-align: left;
                    	color: #000000;">
                Los sistemas SOV cuentan con dos herramientas para brindar un control total: teléfono móvil y mesa de control.
            </h5>
      </div>
    </div>
    <div class="col-sm-3 form-group" style="background-image: url('./images/logo_sov.svg');background-repeat: no-repeat;background-size:70% 70%;padding-top:5%;display: flex;display: -webkit-flex;flex-wrap: wrap;background-position: center;">
        <!--img align="center" src="./images/logo_sov.svg" style="width:100%;height:100%;"></img-->
        <p style="color:transparent"><br>SOV<br></p>
    </div>
  </div>
</div>

<br><br>

<style>
    #titulorecuadroazul{
        font-family: 'Roboto', sans-serif;
        font-weight: bold;
        text-align: center;
        padding-left: 0%;
        color: #ffffff;
    }
    
    #textorecuadrogris{
        font-family: 'Roboto', sans-serif;
        width:90%;
        font-weight: bold;
        text-align: left;
        padding-left:5%;
        padding-right:5%;
        padding-top:5%;
        color: #002E54;
        position: absolute;
        bottom: 0;
        left: 0;
    }
    #textoblanco{
        font-family: 'Roboto', sans-serif;
                  	font-weight: regular;
                  	text-align: left;
                  	padding-left: 10%;
                  	padding-right:10%;
                  	font-size: 17px;
                  	color: #ffffff;"
    }
</style>
<div style="width:100%;" align="center" style="text-align:center;">
    <div class="row" align="center" style="display: flex;width:100%;">
    <div class="col-sm-7 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;">
        <div style="background-color: #002C55;width:100%;">
        <div class="container" style="padding-top:5%;padding-bottom:5%;display: table;overflow: hidden;width:100%;">
            <div style="display: table-cell; vertical-align: middle;">
                <div>
                    <div align="center" style="width:100%;" class="container">
                    <div class="row">
                        <div class="container">
                        <h2 id="titulorecuadroazul">
                      	     SOV RUTAS
                      	</h2>
                        </div>
                    </div>
                    </div>
                    <div class="container">
                        <div class="row">
                            <div class="col-sm-6 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;">
                            <div class="container">
                                <div class="container" style="min-height:102px;">
                                        <div class="container" style="display: inline-block;position: absolute;bottom: 0;left: 0;">
                                            <p id="textoblanco">
                                                OBTENER EL CONTROL</br>TOTAL DE LOS CLIENTES
                                            </p>
                                    </div>
                                </div>
                            </div>
                            </div>
                            <div class="col-sm-6 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;">
                                <div class="container" style="display: inline-block;">
                                    <img src="GRAFICA CIRCULO_0.svg" style="max-width:100%;" align="center"/>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        </div>
    </div>
    <div class="col-sm-5 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;">
      <div class="container" style="width:100%;background-color: #D4D3DA;">
          <div class="row">
              <div class="col-sm-6 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;text-align:center;">
                <div class="container" style="display:inline-block;">
                        <img src="ruta_movil01.gif" align="center" style="padding-top:10%;max-width: 100%;max-height: 100%;"></img>
                </div>
              </div>
              <div class="col-sm-6 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;">
                    <div class="container" style="min-height:210px;display:inline-block;">
                        <div class="container">
                        <h6 align="center" id="textorecuadrogris">
                            CONTEMPLA LAS PRINCIPALES VARIABLES DE RUTAS:</br>
                                        -AUTOVENTA</br>
                                        -PREVENTA</br>
                                        -CALL CENTER</br>
                                        -PORTEO</br>
                            <br>
                      	<form style="text-align: left;padding-left:5%;">
                            <a href="sovrutas.php">
                                <input type="button" href="sovrutas.php" value="VER MÁS" style="width:160px;height:35px;background-color:transparent;color:white;font-size:18px;border-radius: 20px;border: solid 1px #ffffff;"/>
                            </a>
                        </form>
                        </h6>
                        </div>
                    </div>
            </div>   
          </div>
      </div>
    </div>
  </div>
</div>

<br>

<div style="width:100%;" align="center" style="text-align:center;">
    <div class="row flex-column-reverse flex-md-row" align="center" style="display: flex;width:100%;">
     <div class="col-sm-5 col-sm-push-7 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;">
      <div class="container" style="width:100%;background-color: #D4D3DA;">
          <div class="row">
              <div class="col-sm-6 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;text-align:center;">
                <div class="container" style="display:inline-block;">
                    <img src="autotransporte_movil01.gif" align="center" style="padding-top:10%;max-width: 100%;max-height: 100%;"></img>
                </div>
              </div>
              <div class="col-sm-6 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;">
                    <div class="container" style="min-height:120px;display:inline-block;">
                        <div class="container">
                        <h6 align="center" id="textorecuadrogris">
                            MÁS FÁCIL QUE PLUMA Y LÁPIZ, CUMPLE CON LA NORMA 87 
                            <br><br>
                      	<form style="text-align: left;padding-left:5%;">
                            <a href="sovautotransporte.php">
                                <input type="button" href="sovautotransporte.php" value="VER MÁS" style="width:160px;height:35px;background-color:transparent;color:white;font-size:18px;border-radius: 20px;border: solid 1px #ffffff;"/>
                            </a>
                        </form>
                        </h6>
                        </div>
                    </div>
            </div>   
          </div>
      </div>
    </div>
    <div class="col-sm-7 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;">
        <div style="background-color: #002C55;width:100%;">
        <div class="container" style="padding-top:5%;padding-bottom:5%;display: table;overflow: hidden;width:100%;">
            <div style="display: table-cell; vertical-align: middle;">
                <div>
                    <div align="center" style="width:100%;" class="container">
                    <div class="row">
                        <div class="container">
                        <h2 id="titulorecuadroazul">
                      	     SOV AUTOTRANSPORTE
                      	</h2>
                        </div>
                    </div>
                    </div>
                    <br>
                    <div class="container">
                        <div class="row">
                            <div class="container">
                                <p id="textoblanco">
                                  	· Conocer la productividad de la flotilla, mediante gráficos que facilitan la toma de decisiones.</br>
                                    · Estatus en tiempo real de carga de unidad</br>
                                    · Cumple al 100% con la NORMA 87</br>
                                    · Estatus en tiempo real de cada unidad</br>
                                    · ¿Cuánto equivale el tiempo productivo e improductivo de la flotilla? </br>
                                    · En el proceso de carga o descarga, ¿cuál de mis clientes o clientes de mi cliente tarda más las unidades?</br>
                                    · ¿Cuánto tiempo real están invirtiendo mis unidades por incidencias en el camino que retrasan mis viajes? </br>
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
                </div>
            </div>
        </div>
    </div>
  </div>
</div>

<br>

<div style="width:100%;" align="center" style="text-align:center;">
    <div class="row" align="center" style="display: flex;width:100%;">
        <div class="col-sm-7 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;">
        <div style="background-color: #002C55;width:100%;">
        <div class="container" style="padding-top:5%;padding-bottom:5%;display: table;overflow: hidden;width:100%;">
            <div style="display: table-cell; vertical-align: middle;">
                <div>
                    <div align="center" style="width:100%;" class="container">
                    <div class="row">
                        <div class="container">
                        <h2 id="titulorecuadroazul">
                                SOV EVALUACIÓN
                      	    </h2>
                        </div>
                    </div>
                    </div>
                    <br>
                    <div class="container">
                        <div class="row">
                            <div class="container">
                                <div class="container" style="min-height:102px;">
                                        <div class="container" style="display: inline-block;">
                                            <p id="textoblanco">
                                                SISTEMAS MANEJADOS A TRAVÉS DE APLICACIONES.</br><br>
                  	 Obtener el control donde antes era difícil tenerlo. Y calificar el desempeño por funciones. Información específica de las áreas de oportunidad contenidas en las revisiones con información inmediata a las áreas involucradas.
                    
                                            </p>
                                        </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="col-sm-5 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;">
      <div class="container" style="width:100%;background-color: #D4D3DA;">
          <div class="row">
              <div class="col-sm-6 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;text-align:center;">
                <div class="container" style="display:inline-block;">
                        <img src="evaluacion_movil_01.gif" align="center" style="padding-top:10%;max-width: 100%;max-height: 100%;"></img>
                </div>
              </div>
              <div class="col-sm-6 form-group" style="display: flex;display: -webkit-flex;flex-wrap: wrap;">
                    <div class="container" style="min-height:110px;display:inline-block;">
                        <div class="container">
                        <h6 align="center" id="textorecuadrogris">
                            OBTÉN UNA PERSPECTIVA 360° DE LA EVALUACIÓN DE TU NEGOCIO, OBTENIENDO 3 CALIFICACIONES
                            <br><br>
                      	<form style="text-align: left;">
                            <a href="sovevaluacion.php">
                                <input type="button" href="sovrutas.php" value="VER MÁS" style="padding-left:px;width:160px;height:35px;background-color:transparent;color:white;font-size:18px;border-radius: 20px;border: solid 1px #ffffff;"/>
                            </a>
                        </form>
                        </h6>
                        </div>
                    </div>
            </div>   
          </div>
      </div>
    </div>
  </div>
</div>


<br><br>

<div style="height:550px;background-image: url('fondo_parte4.png');background-repeat: no-repeat;background-size:100% 100%;">
    <div class="container" style="height:550px;display: table;overflow: hidden;">
        <div style="display: table-cell; vertical-align: middle;">
            <div>
          <p align="center" style="font-family: 'Roboto', sans-serif;color:white;text-align:center;font-size:28px;">
            Nuestros sistemas convierten tu problemática en ventajas competitivas
          </p>
          <br><br>
          <table align="center" style="text-align:center;">
          <tr>
          <td>
            <div class="container">
                <img style="max-width:100%;" src="icono eficacia_parte4_productos.png"></img>
            </div>
          </td>
          <td>
            <div class="container">
                <img style="max-width:100%;" src="icono utilidades_parte4_productos.png"></img>
            </div>
          </td>
          </tr>
        </table>
      <br><br>
          <h3 style="font-family: 'Roboto', sans-serif;color:white;text-align:center;font-size:35px;width:100%;">
            EFICACIA  =  UTILIDADES
          </h3>
         </div>
     </div>
  </div>
</div>

<br><br><br>

<!-- Footer -->

<?php include "./footer.html" ?>

    <!-- JavaScript -->
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
  </body>
</html>
